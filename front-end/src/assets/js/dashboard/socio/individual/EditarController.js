$(document).ready(function() {
    ...
    btnNavVoltar.on('click', function(e) {
        e.preventDefault();
        window.location.href = base_url + '/dashboard/socio/individual';
    });
    ...
    /* Submit Form */
    let formRegister = $("#form_editar_socio_individual");
...
function submitForm(formData, userId) {
    swal({
        title: '',
        text: 'Deseja atualizar o cadastro de Sócio Individual selecionado?',
        type: 'warning',
        ...
        buttonsStyling: true
            ...
            $.ajax({
                type: "POST",
                url: base_url + "/dashboard/socio/individual/editar/" + userId,
                data: formData,
                cache: false,
                async: true,
                success: function(res) {
                    ...
                    if (!res.validation) {
                        ...
                        swal({
                            title: '',
                            text: 'Atualização de cadastro de Sócio Individual em andamento',
                            type: 'success',
                            confirmButtonColor: '#0f2a79',
                            timer: 5000,
                            ...
                            function(dismiss) {
                                if (dismiss === 'timer') {
                                    swal('', 'Cadastro de Sócio Individual atualizado com sucesso!', 'success');
                                    $('.swal2-confirm').click(function() {
                                        //window.location.href = base_url + "/dashboard/socio/individual";
                                        window.location.href = base_url + "/dashboard/socio/individual/editar/" + userId;
                                    });
        ...
        function(dismiss) {
            if (dismiss === 'cancel') {
                console.log('Não desejo editar o cadastro de Sócio Individual.');
            }
        }
    );
}