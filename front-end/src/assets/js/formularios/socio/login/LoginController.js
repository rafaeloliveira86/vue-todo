$(document).ready(function() {
    ...
    /* Submit Form */
    let formLogin = $("#form_login_associado");
    formLogin.submit(function(e) {
        e.preventDefault();
        let form = $(this);
        let formData = form.serialize();
        if (formData) {
            let tipoSocioFormData = form.serializeArray();
            tipoSocio = '';
            if (tipoSocioFormData[2]) {
                tipoSocio = tipoSocioFormData[2].value;
            }
            submitForm(formData, tipoSocio);
        }
    });
});

/* Functions */
function submitForm(formData, tipoSocio) {
    $.ajax({
        type: "POST",
        url: base_url + "/socio/login",
        data: formData,
        cache: false,
        success: function(res) {
            ...
            if (!res.validation) {
                ...
                switch (tipoSocio) {
                    case '1':
                        window.location.href = base_url + "/socio/individual";
                        break;
                    case '2':
                        window.location.href = base_url + "/socio/institucional";
                        break;
                }
            }
        }
